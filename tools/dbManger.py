...
class DBManger:
    ...
    def updateData(self,CurrentEmail ,**kwargs) -> bool: # updates the user data
        ...
        if user:
            """
            if self.checkPassword(CurrentEmail, password) is False:
                user.password, user.salt = self.hashPassword(password, salt=True)
            """
...
class User(db.Model): # the database structure for the user
